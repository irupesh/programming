...
public class LGCallMonitor {
    ...
    private static final int EVENT_RRC_STATE_CHANGED_SLOT_2 = 7;
    ...
    private boolean mRrcConnected_slot2 = false;
    ...
    private CommandsInterface mCi2 = null;
            ...
            switch(msg.what) {
                ...
                case EVENT_RRC_STATE_CHANGED:
                    ...
                    if (mRrcConnected) {
                        ...
                        log("set persist.vendor.radio.auto_dds_RRCreleased false");
                        SystemProperties.set("persist.vendor.radio.auto_dds_RRCreleased", "false");
                    } else {
                        removeCapSensorCheckAlarm();
                        unregisterCapSensorListener();
                        log("set persist.vendor.radio.auto_dds_RRCreleased true");
                        SystemProperties.set("persist.vendor.radio.auto_dds_RRCreleased", "true");
                    }
                    break;

                case EVENT_RRC_STATE_CHANGED_SLOT_2:
                    ar = (AsyncResult)msg.obj;

                    if (ar.exception != null || ar.result == null) {
                        log("RRC_STATE_CHANGED_SLOT_2 exception!!");
                        break;
                    }
                    mRrcConnected_slot2 = ((int[])ar.result)[0] == 2 ? true : false;
                    log("RRC_STATE_CHANGED_SLOT2, Connection : " + mRrcConnected_slot2);

                    if (mRrcConnected_slot2) {
                        log("set persist.vendor.radio.auto_dds_RRCreleased_SLOT2 false");
                        SystemProperties.set("persist.vendor.radio.auto_dds_RRCreleased_SLOT2", "false");
                    } else {
                        log("set persist.vendor.radio.auto_dds_RRCreleased_SLOT2 true");
                        SystemProperties.set("persist.vendor.radio.auto_dds_RRCreleased_SLOT2", "true");
                    }
                    break;
    ...
    private void initializeForExternalRequest(Context context, CommandsInterface[] cis) {
        if (sExternalRequestAllowed) {
            mContext = context;
            log("Auto DDS Call Monitor initializeForExternalRequest"+cis.length);
    ...
    private void initialize(Context context, CommandsInterface[] cis) {
        ...
        if (sSupportCapacity) {
            if (mCi != null) {
                ...
                if (mCi2 != null)
                    mCi2.registerCapsensorRrcState(mHandler, EVENT_RRC_STATE_CHANGED_SLOT_2, null);
            }
        }
    }
